...
// Vercel's addRandomSuffix (see payload.config.ts) appends a long random
// alphanumeric string right before the extension, e.g.
// "IMG_0894-F2q5MNq0NZ5Lxoh61MWex5dJzykt7m.MOV" — this strips it back off
// so a freshly uploaded file's default Title reads like the name the owner
// actually recognizes ("IMG_0894.MOV") instead of the raw stored filename.
// Heuristic, not a documented Vercel guarantee (the suffix itself is
// generated by Vercel's Blob backend, not the @vercel/blob npm package, so
// its exact shape isn't a contract) — a filename that genuinely already
// ends in "-<15+ alphanumeric chars>" would get over-stripped too. Low
// stakes either way: this only sets the *default* Title, which the owner
// can freely retype regardless (see the "alt" field below).
const RANDOM_SUFFIX_PATTERN = /-[A-Za-z0-9_-]{15,}(\.[^./]+)?$/;
function defaultTitleFromFilename(filename: string): string {
  const stripped = filename.replace(RANDOM_SUFFIX_PATTERN, "$1");
    ...
    beforeChange: [
      ...
      ({ data, operation }) => {
        if (operation === "create" && !data?.alt && data?.filename) {
          data.alt = defaultTitleFromFilename(data.filename);
        }
        return data;
      },
    ],
  ...
  fields: [
    {
      ...
      admin: {
        description:
          ...
          "filename, and in the picker wherever it's used elsewhere. Starts out matching the uploaded " +
          "file's name; feel free to change it to whatever's easiest to recognize. Not shown on the public site.",
      },
    },
  ],
};
